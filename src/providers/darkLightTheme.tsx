...
const darkLightThemeInitialValue = {
  actions: { toggleTheme: () => { } },
  state: { themeMode: 'light' as PaletteType },
};

const DarkLightTheme = React.createContext<DarkLightThemeInitialValue>(darkLightThemeInitialValue);
...
const DarkLightThemeProvider: React.FC = ({ children }) => {
  ...
  const contextData = useMemo(() => ({
    actions: {
      toggleTheme: () => setThemeMode(previousValue => previousValue === 'light' ? 'dark' : 'light'),
    },
    state: { themeMode },
